...
static const char GDSPort1[] = "Arp.Plc.Eclr/test1.Counter1";
static const char GDSPort2[] = "Arp.Plc.Eclr/test1.Counter2";
static const char GDSPort3[] = "Arp.Plc.Eclr/test1.Counter3";
...
void CSampleRuntime::Cycle()
{
	...
	for (;;)
	{
		if(m_bDoCycle)
		{
			...
			Log::Info("{0}: Value: {1}", GDSPort1, m_gdsPort1);
			Log::Info("{0}: Value: {1}", GDSPort2, m_gdsPort2);
			Log::Info("{0}: Value: {1}", GDSPort3, m_gdsPort3);

			Log::Info("Cycles Counter: {0}", cyclesCounter++);
...
bool CSampleRuntime::ReadSubscription()
{
	...
	try
	{
		...
		if(RSCReadVariableValues(m_zSubscriptionValues) == DataAccessError::None)
		{
			...
			if(m_zSubscriptionValues.size() == m_zSubscriptionValues.size())	// sanity-check
			{
...
void PlcOperationHandler(enum PlcOperation operation)
{
	...
	switch (operation)
    {
        ...
        case PlcOperation_StartCold:
        	...
        	// Plc my by stopped by system watchdog so that is possible to start plc cold
			if(g_pRT->Init() == false)
			{
				Log::Error("Error during initialization (Start Cold)");
			}
